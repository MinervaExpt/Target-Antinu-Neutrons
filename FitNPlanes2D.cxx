...
vector<TString> TgtTypes = {"C","Fe","Pb","Water","Other"};
...
int main(int argc, char* argv[]) {
  ...
  if (USDS == "US"){
    ...
    cout << "Argument for upsream v. downstream must be US or DS exactly" << endl;
  ...
  if (Tgt != ""){
    ...
    else if ((TgtNum == 1 || TgtNum ==2 || TgtNum ==5) && material != "Fe" && material != "Pb"){
      cout << "Material Not Valid for Tgt." << endl;
      return 7;
    }
    else if (TgtNum == 4 && material != "Pb"){
      cout << "Material Not Valid for Tgt." << endl;
      return 8;
    }
    else if (TgtNum == 6){
    ...
    else if (TgtNum == 3 && material != "Fe" && material != "Pb" && material != "C"){
      cout << "Material Not Valid for Tgt." << endl;
      return 8;
    }
  }
  else if (material != "C" && material != "Fe" && material != "Pb" && material != "Water"){
    cout << "Invalid Material" << endl;
    return 9;
  }

  cout << "Input Data file name parsed to: " << fileNameStub << endl;
  TFile* outFile = new TFile((TString)(outDir)+"/"+USDS+"vtxFitResults.root","RECREATE");
  ...
  cout << "POT grabbed." << endl;

  double scale = dataPOT->GetVal()/mcPOT->GetVal();

  cout << "Scale Calculated" << endl;
  ...
  TString dirBase = USDS+"_ByType_";

  if (Tgt != "") dirBase = "ByTgt_"+Tgt+"_"+material+"/"+dirBase;
  else if (Tgt == "WaterTgt") dirBase = "ByTgt_"+Tgt;

  TString matName = "";
  if (Tgt != "") matName = Tgt+"_"+material;
  else if (Tgt == "WaterTgt") matName = Tgt;
  else matName = material;

  cout << "starting loop" << endl;
  for (auto tag: tags){
    //cout << dirBase+"Plastic/vtxZ"+tag+"_ByTgt_"+matName+"_PreRecoilCut_Plastic_selected_signal_reco" << endl;
    MnvH1D* h_sig = (MnvH1D*)(mcFile->Get(dirBase+"Plastic/vtxZ"+tag+"_ByTgt_"+matName+"_PreRecoilCut_Plastic_selected_signal_reco"))->Clone();
    cout << "sig" << endl;
    h_sig->Scale(scale);

    MnvH1D* h_1PiC_Bkg = (MnvH1D*)(mcFile->Get(dirBase+"Plastic/vtxZ"+tag+"_ByTgt_"+matName+"_PreRecoilCut_Plastic_background_1chargePi"))->Clone();
    cout << "1piC" << endl;
    h_1PiC_Bkg->Scale(scale);

    MnvH1D* h_1Pi0_Bkg = (MnvH1D*)(mcFile->Get(dirBase+"Plastic/vtxZ"+tag+"_ByTgt_"+matName+"_PreRecoilCut_Plastic_background_1neutPi"))->Clone();
    cout << "1pi0" << endl;
    h_1Pi0_Bkg->Scale(scale);

    MnvH1D* h_NPi_Bkg = (MnvH1D*)(mcFile->Get(dirBase+"Plastic/vtxZ"+tag+"_ByTgt_"+matName+"_PreRecoilCut_Plastic_background_NPi"))->Clone();
    cout << "NPi" << endl;
    h_NPi_Bkg->Scale(scale);

    MnvH1D* h_Other_Bkg = (MnvH1D*)(mcFile->Get(dirBase+"Plastic/vtxZ"+tag+"_ByTgt_"+matName+"_PreRecoilCut_Plastic_background_Other"))->Clone();
    cout << "Other" << endl;
    h_Other_Bkg->Add((MnvH1D*)(mcFile->Get(dirBase+"Plastic/vtxZ"+tag+"_ByTgt_"+matName+"_PreRecoilCut_Plastic_background_Wrong_Nucleus")));
    cout << "Wrong" << endl;
    h_Other_Bkg->Add((MnvH1D*)(mcFile->Get(dirBase+"Plastic/vtxZ"+tag+"_ByTgt_"+matName+"_PreRecoilCut_Plastic_background_DSPlastic")));
    cout << "DS" << endl;
    h_Other_Bkg->Add((MnvH1D*)(mcFile->Get(dirBase+"Plastic/vtxZ"+tag+"_ByTgt_"+matName+"_PreRecoilCut_Plastic_background_USPlastic")));
    cout << "US" << endl;
    ...
    MnvH1D* h_Mat_Bkg = (MnvH1D*)(mcFile->Get(dirBase+material+"/vtxZ"+tag+"_ByTgt_"+matName+"_PreRecoilCut"+mat+"_data"))->Clone();
    cout << "Mat" << endl;
    ...
    for (auto typeName : TgtTypes){
      ...
      h_All_Bkg->Add((MnvH1D*)(mcFile->Get(dirBase+typeName+"/vtxZ"+tag+"_ByTgt_"+matName+"_PreRecoilCut_"+typeName+"_data")));
    }
    h_All_Bkg->Scale(scale);

    MnvH1D* h_data = (MnvH1D*)(dataFile->Get(dirBase+"Other/vtxZ"+tag+"_ByTgt_"+matName+"_PreRecoilCut_Other_data"))->Clone();
    ...
    map<TString,map<TString,MnvH1D*>> result = FitScaleFactorsAndDraw(h_data, fitHists, unfitHists, "NPlanes"+USDS+tag, outDir, "_fit"+USDS, lowBin, hiBin, doSyst, true, varsToSave);
    
    map<TString,MnvH1D*> scaledHists = {};
    scaledHists["Plastic"]=(MnvH1D*)h_Plastic->Clone();
    
    for(auto hists:scaledHists){
      hists.second->Multiply(hists.second,result["NPlanes"+USDS+tag][hists.first]);
      for (auto var:result)var.second[hists.first]->SetDirectory(outFile);
    }
    DrawFromMnvH1Ds(h_data,scaledHists,unfitHists,true,outDir+"TEST_NEW_NPlanes"+USDS+tag+"_postFit");
